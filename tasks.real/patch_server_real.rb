...
OsPatching::OsPatching.refresh_facts(starttime, timeout)
# ...parse the result
facts = {
  values: {
    os: Facter.value(:os),
...
def reboot_required(family, release, reboot)
  ...
  if ['always', 'patched'].include?(reboot)
    ...
    if release.to_i > 6
      ...
      # needs a reboot
      _status, output = OsPatching::OsPatching.run_with_timeout('/usr/bin/needs-restarting')
...
if params['clean_cache'] && params['clean_cache'] == true
  OsPatching::OsPatching.clean_cache(starttime, facts[:values][:os]['family'], timeout)
end
...
if facts[:values][:os]['family'] == 'RedHat'
  ...
  status, stderrout = OsPatching::OsPatching.run_with_timeout("#{deb_front} apt-get #{dpkg_params} -y #{deb_opts} #{apt_mode}", timeout)
